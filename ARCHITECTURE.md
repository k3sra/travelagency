...
src/
  ...
  components/
    ...
    home/                      Hero, Manifesto, Reel, Moments, Kinetic, Destinations, JourneyIndex, CollectiveTeaser, StraightAnswers, Closing
    journey/                   JourneyHeader, Chapters, Collective, SecureSpot
    ui/                        small shared atoms
scripts/
  fetch-media.mjs              Pexels photography and films (sharp crop, blur siblings, film grade)
public/media/                  committed assets (see Media manifest)
```
...
Fetched by `npm run media` (Pexels) into `public/media` (override root with `NEXT_PUBLIC_MEDIA_BASE`).
Films are streamed from Pexels; `src/lib/media-remote.json` records a landscape and a portrait
MP4 (hd and sd) for the hero, the moments reel and every journey.

```
hero/poster.jpg (+ -blur)  hero/poster-tall.jpg (+ -blur)        first frames of the hero films
moments/film.jpg, film-tall.jpg (+ -blur)                         first frames of the reel films
moments/m01..m08.jpg (+ -blur)                                    candid travellers, card and wide frames
journeys/<slug>/hero.jpg, hero-tall.jpg (+ -blur)                 first frames of the journey films
journeys/<slug>/card.jpg (+ -blur)  chapter-01..05.jpg (+ -blur)
portraits/p01..p16.jpg            monochrome, 600×800
curators/c01..c04.jpg             monochrome, 600×800
credits.json                      Pexels attribution, rendered at /credits
```
